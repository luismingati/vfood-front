    ...
    return (
        <div className="flex flex-col gap-[12px] mt-[27px] font-poppins">
            <div className="flex flex-row gap-[12px]">
                {card.contador > 5 ? 
                <p className="text-[#312843] text-[18px]">{alfabeto[card.contador - 65]}</p> : 
                ...
                <div className="flex flex-row flex-wrap gap-[10px] w-max-[860px] ml-[20px]">
                    {
                        cards.map((carta) => {
                            {
                                if(card.contador > 5){
                                    if(carta.name[0].toLowerCase() != alfabeto[card.contador - 65].toLowerCase()) return null;
                                    else return <ColaboratorCard name={carta.name} role={carta.role} stars={carta.stars}/>
                                }
                                else{
                                    if((carta.stars || 0) <= card.contador && (carta.stars || 0) > card.contador - 1) return (<ColaboratorCard name={carta.name} role={carta.role} stars={carta.stars}/>)
                                    return null
                                }
                        }
                        }
